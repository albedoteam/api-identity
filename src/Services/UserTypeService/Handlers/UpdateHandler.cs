...
{
    public class UpdateHandler : CommandHandler<Update, UserType>
    {
        private readonly IRequestClient<UpdateUserType> _client;
        private readonly IUserTypeMapper _mapper;

        public UpdateHandler(IRequestClient<UpdateUserType> client, IUserTypeMapper mapper)
        {
            _client = client;
            _mapper = mapper;
        }

        protected override async Task<Result<UserType>> Handle(Update request)
        {
